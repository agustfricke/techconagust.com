    ...
    <div className="flex items-center gap-2">
      <Listbox value={language} onChange={handleSelect}>
        <div className="relative">
          <Listbox.Button className="relative flex cursor-pointer items-center justify-center rounded-lg border bg-gris-200/50 p-2 text-left focus:outline-none hover:bg-gris-200/75 dark:border-gris-200/20 dark:bg-gris-200/10 dark:hover:border-gris-200/50 sm:text-sm">
            <svg
              viewBox="0 0 88.6 77.3"
              className="h-6 w-6 text-slate-900 dark:text-gris-100"
              role="img"
            >
              <path
                fill="currentColor"
              ...
              {Object.entries(KNOWN_LANGUAGES).map(([code, name]) => (
                <Listbox.Option
                  ...
                  className={({ selected, active }) =>
                    `focus-none relative cursor-pointer bg-gris-200/50 px-4 py-2 text-slate-900 outline-none hover:bg-gris-300/75 dark:bg-gris-200/10 dark:text-gris-100 dark:hover:bg-gris-200/20 ${
                      selected && "bg-gris-400/75 dark:bg-gris-400/20"
                    }
                    ${active && "bg-gris-300/75 dark:bg-gris-200/20"}`
                  }
                  value={code}
                >
                  {({ selected }) => (
